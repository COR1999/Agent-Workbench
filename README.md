...
| [gnu-sed-idioms-empty-files-on-bsd](lessons/gnu-sed-idioms-empty-files-on-bsd.md) | `macos` | GNU-only `sed` label/branch constructs exit 0 and produce an EMPTY file under BSD sed; a script that trims a file this way destroys it on macOS while passing on Linux and Windows |
