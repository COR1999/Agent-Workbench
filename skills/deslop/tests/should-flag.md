...
rewrite**. All from Era-2 repos (`invoiceToSheet`, `kitchenapp`, `fitnessTracker`)
...
### 1 — comment restating the next line · invoiceToSheet `middleware.ts:4`
```ts
...
### 2 — restating comments · invoiceToSheet `src/app/api/upload/route.ts:20`
```ts
    // Validate file type
    ...
...
### 3 — narrating comments over self-evident calls · invoiceToSheet `src/lib/export.ts:5`
```ts
...
### 4 — aspirational production prose · invoiceToSheet `src/lib/email.ts:19`
```ts
...
### 5 — aspirational prose · invoiceToSheet `src/lib/storage.ts:4`
```ts
// In production, you'd use a database like PostgreSQL, MongoDB, etc.
```
**Remove.** Says nothing about what the code does.

### 6 — banner / tutorial comment · fitnessTracker `my-app/constants.ts:15`
```ts
/*
...
### 7 — TODO admitting missing error handling · fitnessTracker `my-app/src/services/workoutStorage.ts:34`
```ts
...
### 8 — commented-out scaffold · fitnessTracker `playwright.config.ts:53`
```ts
    // {
    //   name: 'Mobile Chrome',
    //   use: { ...devices['Pixel 5'] },
...
### 9 — `as any` to silence the compiler · kitchenapp `src/components/Dashboard.tsx:334`
```tsx
...
### 10 — untyped update param · kitchenapp `src/components/Dashboard.tsx:89`
```tsx
