...
Run against real history in `client-commerce` (a private client e-commerce
codebase), at the commit *before* each known fix, scoring the sweep's output
against the files the human fix actually changed. Read-only: a scratch
inspection via `git show <ref>:<path>`, no checkout, no edit to any working
copy. Identifiers are anonymized for publication; full evidence lives in the
private history.

## Case A — silent Supabase errors (known fix: PR touching 10 files)

**Ground truth:** the fix changed 10 files. Pre-fix ref `0feea49`.
...
| admin/list-suppliers.ts | ✅ |
| commerce/storefront.ts | ✅ |
| orders/get-order-summary.ts | ✅ |
| orders/search-order-ids.ts | ✅ |
| **suppliers.ts** | ⚠️ missed on first run, found after scope fix |

**Extra sites the sweep flagged that the human fix did *not* touch** — the interesting part:

| File | Verdict | Reasoning |
|---|---|---|
| `orders/shipping.ts` | **same-class — a real miss** | Swallows a query error. Still unfixed in the current tree. The human sweep behind the fix missed it; this sweep found it. |
...
1. **Glob-pathspec bug (the `suppliers.ts` miss).** `git grep -- 'src/lib/**/*.ts'`
   silently matches **zero** files sitting directly in `src/lib/` — the `**` glob
   requires an intermediate directory. `suppliers.ts` was invisible until the
...
## Case B — unbounded full-table scans (two-issue arc ending in a pagination fix)

**Ground truth:** the tracking issue named `list-customers.ts` and `analytics.ts`
as still scanning after the orders-table pagination fix. Pre-fix ref taken from
history.
